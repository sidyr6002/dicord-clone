...
import { ArrowRight, ArrowRightFromLine, ChevronDown, PlusCircle, Settings, Trash2, UserPlus2, Users } from 'lucide-react'
  ...
  return (
        <DropdownMenu>
            ...
            <DropdownMenuContent className='w-48 text-xs py-2 text-black dark:text-neutral-400 space-y-2'>
                {isModerator && <DropdownMenuItem className='text-sm text-blue-600 dark:text-blue-400 px-2 py-1'>
                    Invite People
                    <UserPlus2 className='w-4 h-4 ml-auto' />
                </DropdownMenuItem>}
                {isAdmin && <DropdownMenuItem className='text-sm px-2 py-1'>
                    Server Settings
                ...
                {isModerator && <DropdownMenuItem className='text-sm px-2 py-1'>
                    Create Channel
                    <PlusCircle className='w-4 h-4 ml-auto' />
                </DropdownMenuItem>}
                {isModerator && <DropdownMenuSeparator />}
                {isAdmin && <DropdownMenuItem className='text-sm text-rose-600 px-2 py-1'>
                    Delete Channel
